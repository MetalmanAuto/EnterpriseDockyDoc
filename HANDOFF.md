...
- **Switching on** (all on Render): `RAZORPAY_KEY_ID` and `RAZORPAY_KEY_SECRET` are already set; add `RAZORPAY_WEBHOOK_SECRET` after creating the webhook in the Razorpay dashboard (URL `https://dockydoc-api-staging.onrender.com/api/v1/billing/webhooks/razorpay`, events: subscription.*, payment.captured, payment.failed). For Paddle add `PADDLE_API_KEY`, `PADDLE_CLIENT_TOKEN`, `PADDLE_WEBHOOK_SECRET` (notification URL `.../api/v1/billing/webhooks/paddle`, all subscription and transaction events) and `PADDLE_ENV=production`. Razorpay subscriptions need the Subscriptions feature enabled on the account. Once the keys are in, the API creates every plan, top-up and storage price at the enabled providers on boot (`PaymentsService.syncPrices`, also `POST /admin/billing/prices/sync`); existing prices are reused, so it is safe to run again.
